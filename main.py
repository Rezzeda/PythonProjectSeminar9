...
def main():
    ...
    find_handler = ConversationHandler(
        ...
        states={
            1: [MessageHandler(Filters.text & ~Filters.command, model.find_abonent)]
        },
    ...
    add_handler = ConversationHandler(
        ...
        states={
            1: [MessageHandler(Filters.text & ~Filters.command, model.add_abonent)]
        },
        fallbacks=[CommandHandler('stop', stop)]
    )
    ...
    delete_handler = ConversationHandler(
        ...
        states={
            1: [MessageHandler(Filters.text & ~Filters.command, model.delete_abonent)]
        },
        fallbacks=[CommandHandler('stop', stop)]
    )
    ...
    update_handler = ConversationHandler(
        ...
        states={
            1: [MessageHandler(Filters.text & ~Filters.command, model.update_info)]
        },
        fallbacks=[CommandHandler('stop', stop)]
    )

    start_handler = CommandHandler('start', start)
    read_handler = CommandHandler('all', model.all)
